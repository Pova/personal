class Sensor {
    constructor(car, rayCount=5, rayLength=150, raySpread=Math.PI/4){
        this.car = car;
        this.rayCount = rayCount;
        this.rayLength = rayLength;
        this.raySpread = raySpread;
        this.rays = [];
        this.readings = []; //distances to obstacle
    }

    sensorUpdate(path,traffic){ 
        this.#castRays();
        this.readings = [];
        for (let i=0;i<this.rays.length;i++){
            this.readings.push(
                this.#getReadings(
                    this.rays[i], 
                    path,
                    traffic)
            );
        }
    }
    
    #castRays(){
        this.rays = [];
        for (let i=0;i<this.rayCount;i++){
            const rayAngle=lerp(
                this.raySpread/2,
                -this.raySpread/2,
                this.rayCount==1?0.5:i/(this.rayCount-1)
            )-this.car.direction;

        const start = {x:this.car.x, y:this.car.y};
        const end = {
            x:this.car.x - Math.sin(rayAngle)*this.rayLength,
            y:this.car.y - Math.cos(rayAngle)*this.rayLength
        };
        this.rays.push([start,end]);
        }
    }

    #getReadings(ray,path,traffic){
        let touches=[];
        for(let i=0;i<path.roads.length;i++){
            let roadBorders=path.roads[i].borders;
            for(let j=0;j<roadBorders.length;j++){
                const touch=getIntersection(
                    ray[0],
                    ray[1],
                    roadBorders[j][0],
                    roadBorders[j][1]
                );
                if(touch){
                    touches.push(touch);
                }
            }

            for (let i=0;i<traffic.vehicles.length;i++){
                const poly=traffic.vehicles[i].getPolygon(); //check this
                for (let j=0;j<poly.length;j++){
                    const value=getIntersection(
                        ray[0],
                        ray[1],
                        poly[j],
                        poly[(j+1)%poly.length]
                    );
                    if (value){
                        touches.push(value);
                    }
                }
            }
        }

        if(touches.length==0){
            return null;
        } else {
            const offsets=touches.map(e=>e.offset); //returns offsets array
            const minOffset=Math.min(...offsets); //... spreads array into values
            return touches.find(e=>e.offset==minOffset);
        }
    }

    draw(ctx){
        for(let i=0;i<this.rayCount;i++){
            let end=this.rays[i][1];
            if (this.readings[i]){
                end=this.readings[i];
            }

            ctx.beginPath();
            ctx.lineWidth=2;
            ctx.strokeStyle="yellow";
            ctx.moveTo(
                this.rays[i][0].x,
                this.rays[i][0].y
            );
            ctx.lineTo(
                end.x,
                end.y
            );
            ctx.stroke();

            ctx.beginPath();
            ctx.lineWidth=2;
            ctx.strokeStyle="black";
            ctx.moveTo(
                this.rays[i][1].x,
                this.rays[i][1].y
            );
            ctx.lineTo(
                end.x,
                end.y
            );
            ctx.stroke();
            
        }
    }
}